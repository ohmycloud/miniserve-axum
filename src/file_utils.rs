...
// means user jas no permission to access the path.
pub fn contains_symlink(path: impl AsRef<Path>) -> io::Result<bool> {
    let contains_symlink = path
        .as_ref()
        .ancestors()
        // On windows, `\\?\` won't exist event though it's the root, but there's no need to check it
        // So we filter it out.
        .filter(|p| p.exists())
...
mod tests {
    ...
    #[case("/.foo")]
    #[case("foo/.bar/foo")]
    fn test_sanitize_path_no_hiden_files(#[case] input: &str) {
