...
pub fn port() -> u16 {
    ...
    loop {
        let ipv4 = TcpListener::bind(("127.0.0.1", 0)).expect("Couldn't find a free local port");
        ...
        if socket.bind(&address.into()).is_ok() {
            return port;
        }
    }
}
    ...
    I::Item: AsRef<std::ffi::OsStr>,
{
    let port = port();
    let tmpdir = tmpdir();
    let mut child = Command::new(cargo::cargo_bin!("miniserve-axum"))
        .arg(tmpdir.path())
        .arg("-v")
        .arg("-p")
        .arg(port.to_string())
